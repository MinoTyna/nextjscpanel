...
export default function DetailClientPage() {
  ...
  const [clientt, setClientt] = useState<ClientData | null>(null);
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/paiement/get/${clientId}`)
      .then((res) => res.json())
      .then((data) => setClientt(data))
  ...
  // Charger données client
  const fetchClientData = async (clientId: number, dateAchat: string) => {
    try {
      const encodedDate = encodeURIComponent(dateAchat);
      const res = await fetch(
        `${
          process.env.NEXT_PUBLIC_BACKEND_URL
        }/paiement/get/${clientId}/${encodedDate}?ts=${Date.now()}`,
        { cache: "no-store" }
      );
      ...
      if (data.paiements && data.paiements.length > 0) {
        setDernierPaiementId(data.paiements[data.paiements.length - 1].id);
      } else {
  ...
  useEffect(() => {
    if (id && dateAchat) {
      ...
      if (!isNaN(numericId)) {
        fetchClientData(numericId, dateAchat);
      }
    }
  }, [id, dateAchat]);
  ...
  const handleVoirFacture = (data: { client_id: number }) => {
  ...
  const nomComplet = `${client.client.nom ?? ""} ${
  ...
  return (
    <div className="bg-gradient-to-br from-red-400/30 to-blue-600/90 w-full min-h-screen p-4">
        ...
        <div>
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => handleVoirFacture({ client_id: clientId! })}
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    setShowUpdateModal(false);
                    await fetchClientData(clientId, dateAchat); // Refresh
                  } catch (err) {
